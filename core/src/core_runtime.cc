...
void Runtime::Create(hipc::FullPtr<CreateTask> task, chi::RunContext &ctx) {
  ...
  if (!storage_devices_.empty()) {
    ...
    for (size_t i = 0; i < storage_devices_.size(); ++i) {
      ...
      // Convert string bdev_type to enum
      ...
      // Use the CTE client API to register the target
      chi::u32 result = client_.RegisterTarget(HSHM_MCTX, target_name, bdev_type, 
                                              device.path_, device.capacity_limit_);
      
      if (result == 0) {
        std::cout << "  - Registered target '" << target_name << "': " << device.path_ 
                  << " (" << device.bdev_type_ << ", " 
  ...
  CreateLocalQueue(kStatsQueue, 
                   config.stats_queue_.lane_count_,
  ...
  if (config_loaded) {
    if (!config_path.empty()) {
      std::cout << "Configuration loaded from file: " << config_path << std::endl;
    } else {
...
void Runtime::RegisterTarget(hipc::FullPtr<RegisterTargetTask> task,
  ...
  try {
    ...
    // Create bdev for this target
    // Use default constructor to let the runtime generate pool ID
    chimaera::bdev::Client bdev_client;
    
    // Create the bdev container
    bdev_client.Create(HSHM_MCTX, chi::PoolQuery::Local(), bdev_type, file_path, total_size);
    ...
    target_info.bdev_client_ = std::move(bdev_client);  // Store the bdev client
...
void Runtime::StatTargets(hipc::FullPtr<StatTargetsTask> task,
  ...
  try {
    ...
    locks.reserve(target_locks_.size());
    for (size_t i = 0; i < target_locks_.size(); ++i) {
      locks.emplace_back(std::make_unique<chi::ScopedCoRwWriteLock>(*target_locks_[i]));
    }
...
void Runtime::GetOrCreateTag(hipc::FullPtr<GetOrCreateTagTask> task,
                             chi::RunContext &ctx) {
  try {
...
void Runtime::MonitorGetOrCreateTag(chi::MonitorModeId mode,
...
void Runtime::PutBlob(hipc::FullPtr<PutBlobTask> task, chi::RunContext &ctx) {
  try {
    ...
    if (size == 0) {
      task->result_code_ = 1;
      task->error_message_ = chi::string(main_allocator_, "Blob size cannot be zero");
      return;
    }

    if (blob_data.IsNull()) {
      task->result_code_ = 1;
      task->error_message_ = chi::string(main_allocator_, "Blob data pointer is null");
      return;
    }
    ...
    if (available_targets.empty()) {
      task->result_code_ = 1;
      task->error_message_ = chi::string(main_allocator_, "No targets available for data placement");
      return;
    }

    // Create Data Placement Engine based on configuration
    const Config &config = GetConfig();
    std::unique_ptr<DataPlacementEngine> dpe = DpeFactory::CreateDpe(config.dpe_.dpe_type_);
    
    // Select target using DPE algorithm
    std::string selected_target = dpe->SelectTarget(available_targets, blob_score, size);
    ...
    TargetInfo *target_info = nullptr;
    {
      ...
      if (target_it == registered_targets_.end()) {
        task->result_code_ = 1;
        task->error_message_ = chi::string(main_allocator_, 
            "Selected target '" + selected_target + "' no longer exists");
        return;
      }
      ...
      if (target_info->remaining_space_ < size) {
        task->result_code_ = 1;
        task->error_message_ = chi::string(main_allocator_, 
            "Selected target '" + selected_target + "' no longer has sufficient space");
        return;
      }
    ...
    // TODO: Implement actual bdev write operation using target_info->bdev_client_
    ...
    std::cout << "PutBlob: blob_id=" << blob_id << ", name=" << blob_name 
              ...
              << ", score=" << blob_score << ", dpe=" << config.dpe_.dpe_type_ << std::endl;
...
void Runtime::GetBlob(hipc::FullPtr<GetBlobTask> task, chi::RunContext &ctx) {
  try {
    ...
    if (size == 0) {
      task->result_code_ = 1;
      task->error_message_ = chi::string(main_allocator_, "Read size cannot be zero");
      return;
    }
    ...
    if (!found) {
      task->result_code_ = 1;
      task->error_message_ = chi::string(main_allocator_, 
          "Blob not found (tag_id=" + std::to_string(tag_id) + 
    ...
    if (offset >= blob_info.size_) {
      ...
      task->error_message_ = chi::string(main_allocator_, 
          ...
          " exceeds blob size " + std::to_string(blob_info.size_));
      return;
    }
    ...
    // TODO: Perform async read operation from bdev using target_info.bdev_client_
    // This would involve:
    // 1. target_info.bdev_client_.Read(target_offset, actual_size, task->blob_data_);
    // 2. Handle multi-block reads if blob spans multiple blocks
    // 3. Async operation completion handling
    ...
    std::cout << "GetBlob: blob_id=" << blob_info.blob_id_ 
              ...
              << ", size=" << actual_size
              << " (requested=" << size << ", blob_size=" << blob_info.size_ << ")" << std::endl;

    task->result_code_ = 0; // Success
    
    // Update task with actual read parameters
    task->size_ = actual_size; // Update size to reflect actual bytes that will be read
...
std::string Runtime::CreateBdevForTarget(const std::string &target_name,
  ...
  std::cout << "CreateBdevForTarget: target=" << target_name
            ...
            << ", capacity=" << device_config.capacity_limit_ << " bytes" << std::endl;
...
void Runtime::UpdateTargetStats(const std::string &target_name,
  ...
  // TODO: Implement actual stats polling from bdev using target_info.bdev_client_
  // For now, increment some mock stats using configured parameters
  target_info.ops_read_ += 1;
  target_info.ops_written_ += 1;
  
  // Use configured cache size to simulate read/write amounts
  chi::u64 read_amount = std::min(static_cast<chi::u64>(4096), 
  ...
  // Mock latency calculation with performance consideration
  float mock_latency = 50.0f * (1.0f / config.performance_.max_concurrent_operations_);
  target_info.avg_latency_us_ = (target_info.avg_latency_us_ + mock_latency) / 2.0f;
  
  // Calculate current bandwidth in MB/s (mock values for now)
  // TODO: Use actual performance metrics from bdev client
  double time_elapsed_s = 1.0; // Assume 1 second elapsed for calculation
  target_info.read_bandwidth_mbps_ = (target_info.bytes_read_ / (1024.0 * 1024.0)) / time_elapsed_s;
  target_info.write_bandwidth_mbps_ = (target_info.bytes_written_ / (1024.0 * 1024.0)) / time_elapsed_s;
  
  // Auto-calculate target score using normalized log bandwidth
  double max_bandwidth = std::max(target_info.read_bandwidth_mbps_, target_info.write_bandwidth_mbps_);
  if (max_bandwidth > 0.0) {
    // Find the maximum bandwidth across all targets for normalization
    double global_max_bandwidth = 1000.0; // TODO: Calculate actual max from all targets
    
    // Use logarithmic scaling for target score: log(bandwidth_i) / log(bandwidth_MAX)
    target_info.target_score_ = static_cast<float>(
        std::log(max_bandwidth + 1.0) / std::log(global_max_bandwidth + 1.0)
    );
    
    // Clamp to [0, 1] range
    target_info.target_score_ = std::max(0.0f, std::min(1.0f, target_info.target_score_));
  } else {
...
chi::u32 Runtime::GetOrAssignBlobId(chi::u32 tag_id,
                                    ...
                                    chi::u32 preferred_id) {
  size_t tag_lock_index = GetTagLockIndex(blob_name);  // Use blob_name for lock selection
  chi::ScopedCoRwWriteLock write_lock(*tag_locks_[tag_lock_index]);
