...
1. **Environment Variable (Recommended)**: `WRP_RUNTIME_CONF` or `WRP_RUNTIME_CONF`
   - Points to a YAML configuration file
   - Most flexible and explicit method
   - `WRP_RUNTIME_CONF` is checked first, then `WRP_RUNTIME_CONF`
...
1. **WRP_RUNTIME_CONF** (checked first)
2. **WRP_RUNTIME_CONF** (fallback if WRP_RUNTIME_CONF is not set)
3. Built-in defaults (if neither environment variable is set)

**Examples**:

```bash
# Method 1: Using WRP_RUNTIME_CONF (recommended)
export WRP_RUNTIME_CONF=/etc/chimaera/chimaera_config.yaml
chimaera_start_runtime
...
|----------|-------------|---------|----------|
| `WRP_RUNTIME_CONF` | Path to YAML configuration file | (empty - uses defaults) | Primary |
| `WRP_RUNTIME_CONF` | Alternative path to YAML configuration file | (empty - uses defaults) | Secondary |

**Note**: The runtime checks `WRP_RUNTIME_CONF` first. If not set, it falls back to `WRP_RUNTIME_CONF`. If neither is set, built-in defaults are used.

**Important**: The runtime does NOT read individual `CHI_*` environment variables (like `CHI_SCHED_WORKERS`, `CHI_ZMQ_PORT`, etc.). All configuration must be specified in a YAML file pointed to by `WRP_RUNTIME_CONF` or `WRP_RUNTIME_CONF`.
...
**Solutions**:
1. Ensure `WRP_RUNTIME_CONF` or `WRP_RUNTIME_CONF` is set before starting runtime:
   ```bash
...
- **Runtime startup**: `util/chimaera_start_runtime.cc`
  ...
  - Loads configuration from WRP_RUNTIME_CONF or WRP_RUNTIME_CONF
