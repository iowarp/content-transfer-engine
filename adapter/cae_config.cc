...
bool CaeConfig::LoadFromFile(const std::string& config_path) {
  ...
  try {
    ...
    HELOG(kError, "Failed to load CAE config from file {}: {}", config_path, e.what());
    return false;
  }
}
...
bool CaeConfig::SaveToFile(const std::string& config_path) const {
  ...
  try {
    ...
    if (!file.is_open()) {
      HELOG(kError, "Failed to open CAE config file for writing: {}", config_path);
      return false;
    }
    ...
    HELOG(kError, "Failed to save CAE config to file {}: {}", config_path, e.what());
    return false;
  }
}
