...
# Find and remove Makefiles (excluding build directory)
echo "Removing Makefiles..."
find . -name "Makefile" -type f | grep -v "^./build/" | while read -r file; do
...
find . -name "CTestTestfile.cmake" -type f | grep -v "^./build/" | while read -r file; do
    $SUDO chmod +w "$file" 2>/dev/null || true
    $SUDO rm -f "$file" && echo "  Removed: $file"
done
...
find . -name "CMakeCache.txt" -type f | grep -v "^./build/" | while read -r file; do
    $SUDO chmod +w "$file" 2>/dev/null || true
    $SUDO rm -f "$file" && echo "  Removed: $file"
done
...
find . -name "cmake_install.cmake" -type f | grep -v "^./build/" | while read -r file; do
    $SUDO chmod +w "$file" 2>/dev/null || true
    $SUDO rm -f "$file" && echo "  Removed: $file"
done
...
find . -name "CMakeFiles" -type d | grep -v "^./build/" | while read -r dir; do
...
find . -name "DartConfiguration.tcl" -type f | grep -v "^./build/" | while read -r file; do
    $SUDO chmod +w "$file" 2>/dev/null || true
    $SUDO rm -f "$file" && echo "  Removed: $file"
done
...
find . -name "Testing" -type d | grep -v "^./build/" | while read -r dir; do
    $SUDO chmod -R +w "$dir" 2>/dev/null || true
    $SUDO rm -rf "$dir" && echo "  Removed: $dir"
done
